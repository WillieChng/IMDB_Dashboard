...
def basic():
    ...
    fig2 = px.treemap(df2, path=['director'], values='vote_count', color='popularity', width=600, height=400)
    chart2 = pio.to_html(fig2, full_html=False)
    
    ##CHART 3: Genre Distribution
    df3 = df.groupby('genre').size().reset_index(name="count")
    fig3 = px.pie(df3, values="count", names="genre", width=600, height=400)
    ...
    fig4 = px.line(df4, x='release_year', y='count', width=600, height=400)
    ...
    fig5 = px.bar(df5, x='adult', y='count', width=600, height=400)
